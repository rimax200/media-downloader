...
final class SettingsWindowController: NSWindowController, NSWindowDelegate {
    ...
    init(preferences: PreferencesStore, onCheckForUpdates: @escaping () -> Void) {
        ...
        let contentSize = NSSize(width: 560, height: 530)
        let window = NSWindow(
...
private final class SettingsRootView: NSView {
    ...
    init(frame: NSRect, preferences: PreferencesStore, onCheckForUpdates: @escaping () -> Void) {
        ...
        let qualityTitle = sectionTitle("Download Quality")
        let qualityCard = QualitySettingsCard(preferences: preferences)
        let shortcutsTitle = sectionTitle("Shortcuts")
        let shortcutsCard = ShortcutSettingsCard(preferences: preferences)

        [generalTitle, generalCard, qualityTitle, qualityCard, shortcutsTitle, shortcutsCard].forEach {
        ...
        NSLayoutConstraint.activate([
            ...
            qualityTitle.leadingAnchor.constraint(equalTo: leadingAnchor, constant: Metrics.leading),
            qualityTitle.topAnchor.constraint(equalTo: generalCard.bottomAnchor, constant: 30),

            qualityCard.leadingAnchor.constraint(equalTo: leadingAnchor, constant: Metrics.leading),
            qualityCard.trailingAnchor.constraint(equalTo: trailingAnchor, constant: -Metrics.leading),
            qualityCard.topAnchor.constraint(equalTo: qualityTitle.bottomAnchor, constant: 20),
            qualityCard.heightAnchor.constraint(equalToConstant: 76),

            shortcutsTitle.leadingAnchor.constraint(equalTo: leadingAnchor, constant: Metrics.leading),
            shortcutsTitle.topAnchor.constraint(equalTo: qualityCard.bottomAnchor, constant: 30),
...
private final class QualitySettingsCard: NSView {
    private let preferences: PreferencesStore
    private let popup = NSPopUpButton(frame: .zero, pullsDown: false)
    private let alwaysUseCheckbox = NSButton(checkboxWithTitle: "Always use this quality (don't ask)", target: nil, action: nil)
    ...
    init(preferences: PreferencesStore) {
        ...
        for quality in VideoQuality.allCases {
            ...
            popup.lastItem?.representedObject = quality
        }
        let savedIndex = VideoQuality.allCases.firstIndex(of: preferences.defaultQuality) ?? 0
        popup.selectItem(at: savedIndex)
        popup.target = self
        popup.action = #selector(qualityChanged)
        popup.translatesAutoresizingMaskIntoConstraints = false
        popup.controlSize = .regular
        popup.bezelStyle = .rounded

        alwaysUseCheckbox.state = preferences.alwaysUseDefaultQuality ? .on : .off
        alwaysUseCheckbox.target = self
        alwaysUseCheckbox.action = #selector(alwaysUseChanged)
        alwaysUseCheckbox.translatesAutoresizingMaskIntoConstraints = false
        alwaysUseCheckbox.font = .systemFont(ofSize: 13)
        alwaysUseCheckbox.contentTintColor = .labelColor

        let qualityLabel = NSTextField(labelWithString: "Default quality")
        qualityLabel.font = .systemFont(ofSize: 13, weight: .regular)
        qualityLabel.textColor = .labelColor
        qualityLabel.translatesAutoresizingMaskIntoConstraints = false

        [qualityLabel, popup, alwaysUseCheckbox].forEach(addSubview)

        NSLayoutConstraint.activate([
            qualityLabel.leadingAnchor.constraint(equalTo: leadingAnchor, constant: 22),
            qualityLabel.centerYAnchor.constraint(equalTo: topAnchor, constant: 24),

            popup.trailingAnchor.constraint(equalTo: trailingAnchor, constant: -22),
            popup.centerYAnchor.constraint(equalTo: qualityLabel.centerYAnchor),
            popup.widthAnchor.constraint(equalToConstant: 180),

            alwaysUseCheckbox.leadingAnchor.constraint(equalTo: leadingAnchor, constant: 22),
            alwaysUseCheckbox.topAnchor.constraint(equalTo: qualityLabel.bottomAnchor, constant: 14),
        ])
    }
    ...
    @objc private func qualityChanged() {
        guard let quality = popup.selectedItem?.representedObject as? VideoQuality else { return }
        preferences.defaultQuality = quality
    }

    @objc private func alwaysUseChanged() {
        preferences.alwaysUseDefaultQuality = alwaysUseCheckbox.state == .on
    }
}
