...
struct ContentView: View {
    ...
    @State private var qualityPickerAppeared = false
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 12) {
                ...
                if model.pendingQualitySelection != nil {
                    QualityPickerPanel(
                        selectedQuality: Binding(
                            get: { model.pendingQualitySelection?.selectedQuality ?? .best },
                            set: { model.pendingQualitySelection?.selectedQuality = $0 }
                        ),
                        onConfirm: model.confirmQualityAndDownload,
                        onCancel: model.cancelQualitySelection
                    )
                    .opacity(qualityPickerAppeared ? 1 : 0)
                    .scaleEffect(qualityPickerAppeared ? 1 : 0.985, anchor: .top)
                    .blur(radius: qualityPickerAppeared ? 0 : 7)
                    .offset(y: qualityPickerAppeared ? 0 : -8)
                }
                ...
                if let session = displayedTrimSession {
                    VideoTrimPanelView(
                        ...
                        onCopy: { selection, progress in try await model.copyActiveTrim(selection, onProgress: progress) },
                        onSave: { selection, url, progress in try await model.saveActiveTrim(selection, to: url, onProgress: progress) }
                    )
                    .id(session.id)
        ...
        .onChange(of: model.pendingQualitySelection == nil) { _, isNil in
            withAnimation(.easeOut(duration: 0.18)) {
                qualityPickerAppeared = !isNil
            }
        }
        .onAppear {
    ...
    private func findInputField(in view: NSView, depth: Int = 0) -> PastingTextField? {
        ...
        for subview in view.subviews {
            if let field = findInputField(in: subview, depth: depth + 1) {
                return field
            }
        }
                ...
                Button {
                    ...
                    VStack(spacing: 4) {
                        ...
                        Text(quality.displayName)
                            .font(.system(size: 13, weight: selectedQuality == quality ? .semibold : .regular))
                            .foregroundStyle(selectedQuality == quality ? Color.primary : Color.primary.opacity(0.6))
                    }
